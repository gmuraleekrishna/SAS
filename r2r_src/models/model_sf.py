...
class EncoderLSTM(nn.Module):
    ''' Encodes navigation instructions, returning hidden state V (for
    ...
    def forward(self, inputs, lengths):
        ''' Expects x vocab indices as (batch, seq_len). Also requires a
...
class AttnDecoderLSTM(nn.Module):
    ...
    def forward(self, action, feature, cand_feat,
        ...
        Q: batch x cand x (feature_size + angle_feat_size)
...
class SpeakerEncoder(nn.Module):
    ...
    def forward(self, action_embeds, feature, lengths, already_dropfeat=False):
        ...
        :return: V with shape (batch_size, length, hidden_size)
        """
        x = action_embeds
        if not already_dropfeat:
