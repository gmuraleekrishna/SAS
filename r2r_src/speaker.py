...
import os
import utils
# from r2r_src.models import model_sf
import torch.nn.functional as F

from models.model_sf import SpeakerEncoder, SpeakerDecoder
...
class Speaker:
    ...
    def __init__(self, env, listener, tok):
        ...
        self.encoder = SpeakerEncoder(self.feature_size + args.angle_feat_size, args.rnn_dim, args.dropout,
        ...
        self.encoder_optimizer = args.optimizer(self.encoder.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        self.decoder_optimizer = args.optimizer(self.decoder.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    def gt_words(self, obs):
        ...
        act_insts = np.array([ob['short_instr_encodings'] for ob in obs])
        act_insts_lens = [len(insts) for insts in act_insts]
        ...
        return torch.from_numpy(full_instr_seq_tensor).cuda(), \
            [torch.from_numpy(np.stack(act_inst)).cuda() for act_inst in act_insts], act_insts_lens

    def teacher_forcing(self, train=True, features=None, insts=None, for_listener=False, train_small=False):
        if train:
            self.encoder.train()
            self.decoder.train()
        else:
        ...
        # Get Language Input
        if insts is None:
            insts, acts_insts, act_inst_lens = self.gt_words(obs)  # Language
            # Feature
        # Decode
        logits, _, _, _ = self.decoder(insts, ctx, ctx_mask, h_t, c_t)
        short_loss = 0.
        if train_small:
            for bid, (act_inst, leng) in enumerate(zip(acts_insts, act_inst_lens)):
                ah_t = torch.zeros(1, leng, args.rnn_dim).cuda()
                ac_t = torch.zeros(1, leng, args.rnn_dim).cuda()
                short_logits, _, _, summary_x = self.decoder(act_inst, ctx[bid].unsqueeze(0),
                                                             ctx_mask[bid].unsqueeze(0), ah_t, ac_t)
                summary_x = summary_x.permute(0, 2, 1).contiguous()
                if for_listener:
                    short_loss += self.nonreduced_softmax_loss(
                        input=summary_x[:, :, :-1],
                        target=act_inst[:, 1:]
                    )
                else:
                    short_loss += self.softmax_loss(
                        input=summary_x[:, :, :-1],
                        target=act_inst[:, 1:]
                    )
        ...
        loss = 0.7 * loss + 0.3 * short_loss

        if for_listener:
            return (0.7 * self.nonreduced_softmax_loss(
                input=logits[:, :, :-1],  # -1 for aligning
                target=insts[:, 1:]  # "1:" to ignore the word <BOS>
            )) + (0.3 * short_loss)
    ...
    def infer_batch(self, sampling=False, train=False, featdropmask=None):
        ...
        (img_feats, can_feats), lengths = self.from_shortest_path(
            viewpoints=viewpoints_list)  # Image Feature (from the shortest path)
