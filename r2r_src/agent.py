...
import utils
import models.model_sf as model
...
class Seq2SeqAgent(BaseAgent):
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        if speaker is not None:  # Trigger the self_train mode!
            ...
            insts = speaker.infer_batch(obs, featdropmask=noise)  # Use the same drop mask in speaker
        ...
        # Reorder the language x for the encoder (do not ruin the original code)
        ...
        if train_rl:
            ...
            for t in range(length - 1, -1, -1):
                ...
                # r_: The higher, the better. -ln(top_p(action)) * (discount_reward - value)
    ...
    def beam_search(self, speaker):
        ...
        for result in results:
            ...
            speaker_scores = speaker.teacher_forcing(train=True, features=features, insts=insts, reduce=True)
            for j, path in enumerate(result['paths']):
